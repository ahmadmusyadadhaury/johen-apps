...
class AsetMesSyncService
{
    private const CACHE_KEY = 'aset_mes_api_payload';
    ...
    {
        $url = config('services.aset_mes_api.url');
        ...
        try {
            $request = Http::timeout(15)
                ->withToken(config('services.aset_mes_api.token'))
                ->acceptJson();

            if (! config('services.aset_mes_api.verify_ssl')) {
                $request = $request->withoutVerifying();
            }

            $response = $request->get($url.config('services.aset_mes_api.path'));

            if (! $response->successful()) {
                Log::warning('AsetMesSyncService: API gagal', [
            ...
            Log::warning('AsetMesSyncService: exception', ['message' => $e->getMessage()]);
            report($e);
    ...
    {
        $id = $item['id'] ?? null;
        $code = $id !== null ? 'MES-'.$id : (string) ($item['code'] ?? '');
        ...
        return [
            ...
            'purchase_date' => null,
            'supplier' => null,
            'location' => $this->nullable($item['keterangan'] ?? null),
            'condition' => 'baik',
    ...
    private function extractMetadata(array $item): array
    {
        return [
            ...
            'kategori' => $this->nullable($item['kategori'] ?? null),
    ...
    private function ensureCategory(): ?AssetCategory
    {
        return AssetCategory::firstOrCreate(
            ['name' => 'Aset Mes'],
            ['description' => 'Data aset mes tersinkron dari office.johengaming.store', 'is_active' => true]
        );
    }
