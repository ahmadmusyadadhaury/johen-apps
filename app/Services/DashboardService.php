...
class DashboardService
{
    ...
    public function getActivityChartData(int $days = 7): array
    {
        ...
        $perDay = fn ($query) => $query
            ->selectRaw('DATE(created_at) as tanggal, COUNT(*) as total')
            ->groupBy('tanggal')
            ->pluck('total', 'tanggal');

        $meetings = $perDay(Meeting::where('created_at', '>=', $start));
        $pengajuan = $perDay(LeaveRequest::where('created_at', '>=', $start));
        $sistem = $perDay(EmailLog::where('created_at', '>=', $start));

        $presensi = Attendance::where('date', '>=', $start->toDateString())
            ->selectRaw('DATE(date) as tanggal, COUNT(*) as total')
            ->groupBy('tanggal')
            ->pluck('total', 'tanggal');

        $build = fn ($counts) => collect(range($days - 1, 0))
            ->map(fn ($i) => (int) ($counts[now()->subDays($i)->toDateString()] ?? 0))
            ->values()
            ->all();
        ...
        return [
            ...
            'labels' => $labels,
            'series' => [
                ['name' => 'Meeting', 'color' => '#14b8a6', 'data' => $build($meetings)],
                ['name' => 'Pengajuan', 'color' => '#8b5cf6', 'data' => $build($pengajuan)],
                ['name' => 'Presensi', 'color' => '#3b82f6', 'data' => $build($presensi)],
                ['name' => 'Aktivitas Sistem', 'color' => '#f59e0b', 'data' => $build($sistem)],
            ],
        ];
    }
    ...
    public function getRecentActivities(int $limit = 8): array
    {
        ...
        Meeting::with('creator')
            ...
            ->take($limit)
            ->get()
            ->each(function ($meeting) use ($activities) {
                $name = $meeting->creator?->name ?? 'Sistem';
                $activities->push([
                    'type' => 'meeting',
                    'user' => $name,
                    'title' => $name.' membuat meeting baru',
        ...
        LeaveRequest::with('employee')
            ...
            ->each(function ($leave) use ($activities) {
                $name = $leave->employee?->nama ?? '-';
                $jenis = $leave->jenis === 'cuti_tahunan' ? 'cuti' : 'izin';
                $activities->push([
                    'type' => 'pengajuan',
                    'user' => $name,
                    'title' => $name.' mengajukan '.$jenis,
                    'detail' => ucfirst(str_replace('_', ' ', $leave->jenis)).' • '.$leave->tanggal_mulai->isoFormat('D MMM').' - '.$leave->tanggal_selesai->isoFormat('D MMM YYYY'),
                    'at' => $leave->created_at,
                ]);
            });
        ...
        Asset::with('creator')
            ...
            ->each(function ($asset) use ($activities) {
                $name = $asset->creator?->name ?? 'Sistem';
                $activities->push([
                    'type' => 'asset',
                    'user' => $name,
                    'title' => $name.' menambahkan data asset',
                    'detail' => $asset->name,
                    'at' => $asset->created_at,
                ]);
            });

        PaymentSubmission::with('pengaju')
            ->latest('created_at')
            ->take($limit)
            ->get()
            ->each(function ($submission) use ($activities) {
                $name = $submission->pengaju?->name ?? '-';
                $activities->push([
                    'type' => 'pembayaran',
                    'user' => $name,
                    'title' => $name.' mengajukan pembayaran',
                    'detail' => ucfirst($submission->jenis).' • Rp '.number_format((float) $submission->nominal, 0, ',', '.'),
        ...
        Attendance::with('employee')
            ...
            ->take($limit)
            ->get()
            ->each(function ($attendance) use ($activities) {
                $name = $attendance->employee?->nama ?? '-';
                $activities->push([
                    'type' => 'presensi',
                    'user' => $name,
                    'title' => $name.' melakukan presensi masuk',
                    'detail' => 'Jam masuk '.Carbon::parse($attendance->time_in)->format('H:i'),
                    'at' => Carbon::parse($attendance->date->toDateString().' '.$attendance->time_in),
                ]);
            });
        ...
        return $activities
            ->sortByDesc('at')
            ->take($limit)
            ->map(fn ($activity) => array_merge($activity, [
                'time_ago' => $activity['at']->diffForHumans(),
                'at_full' => $activity['at']->isoFormat('dddd, D MMMM YYYY HH:mm'),
            ]))
            ->values()
            ->toArray();
    }
