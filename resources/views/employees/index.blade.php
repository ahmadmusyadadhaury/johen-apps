@push('topbar-left')
    <div>
        <h1 class="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100">{{ auth()->user()->pegawaiLabel('Karyawan') }}</h1>
        <p class="hidden sm:block text-xs text-gray-400 mt-0.5">{{ auth()->user()->pegawaiLabel('Kelola data master karyawan') }}</p>
    </div>
@endpush

<x-app-layout title="{{ auth()->user()->pegawaiLabel('Karyawan') }}">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div class="stat-card group">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ auth()->user()->pegawaiLabel('Total Karyawan') }}</p>
        </div>

        <div class="stat-card group">
            <div class="flex items-center justify-between mb-3">
                <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-rose-500 to-red-500 text-white group-hover:scale-110 transition-transform duration-300">
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ auth()->user()->pegawaiLabel('Mantan Karyawan') }}</p>
        </div>
    </div>
