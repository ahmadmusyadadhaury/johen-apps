<div x-data="{ showFotoModal: false, fotoModalUrl: '', fotoModalLabel: '', showFeedbackModal: false, feedbackId: null }">
    <div class="flex items-center justify-between gap-4 mb-6">
        ...
        <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold {{ match($divisi) {
            ...
            'Roblox' => 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-400',
            'Monkey PUBG' => 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400',
            'FC Mobile' => 'bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-400',
                ...
                <tbody class="divide-y divide-gray-50 dark:divide-gray-800">
                    @forelse($groupedItems as $date => $dateItems)
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
                            @if($loop->first)
                            <td class="table-cell text-gray-700 dark:text-gray-300 font-medium text-center" rowspan="{{ $dateItems->count() }}">{{ $item->tanggal->format('d/m/Y') }}</td>
                            @endif
    ...
    <div x-data="{ open: false }"
         ...
         x-transition:leave-start="opacity-100"
         x-transition:leave-end="opacity-0"
         class="fixed inset-0 z-[250] flex items-center justify-center p-5 bg-gray-900/60 backdrop-blur-sm"
         @click="open = false">
        <div x-show="open" x-cloak
             ...
             x-transition:leave-end="opacity-0 scale-95"
             @click.stop
             class="relative w-full max-w-sm rounded-2xl bg-white dark:bg-gray-800 p-6 sm:p-8 text-center shadow-2xl my-10">
