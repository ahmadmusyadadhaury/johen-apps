@push('topbar-left')
    <div class="hidden md:block">
        <h1 class="text-base sm:text-lg font-bold text-gray-900 dark:text-gray-100 truncate">Cuti & Izin</h1>
...
<div x-data="{ confirmAction: false, confirmTitle: '', confirmMessage: '', confirmHandler: null, filtersOpen: false, detailOpen: false, selected: null }">
    ...
    @include('livewire.partials.cuti-izin-mobile-header')
    ...
    <div class="mb-6 hidden md:block">
        <div class="flex sm:inline-flex items-center gap-1 rounded-xl bg-gray-100 dark:bg-gray-800 p-1">
            <button wire:click="$set('tab', 'saya')"
                class="flex-1 sm:flex-none flex items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200 {{ $tab === 'saya' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300' }}">
            ...
            <button wire:click="$set('tab', 'tim')"
                class="flex-1 sm:flex-none flex items-center justify-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 text-sm font-medium transition-all duration-200 {{ $tab === 'tim' ? 'bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300' }}">
                ...
                @if($timMenungguCount > 0)
                    <span class="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1.5 text-[10px] font-bold tabular-nums leading-none text-white shadow-sm">{{ $timMenungguCount }}</span>
                @endif
            </button>
        </div>
    </div>
    @endif
        ...
        <div class="cuti-stats-group grid grid-cols-1 md:grid-cols-2 {{ $jatahAvailable ? 'lg:grid-cols-4' : 'lg:grid-cols-3' }} gap-0 md:gap-5 mb-6 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 md:rounded-none md:border-0 md:bg-transparent md:p-0 md:shadow-none">
            <div class="cuti-stats-item stat-card group">
            ...
            <div class="cuti-stats-item stat-card group relative overflow-hidden">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-500 to-green-500 text-white shadow-lg shadow-emerald-200 group-hover:scale-110 transition-transform duration-300">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 8.25v-1.5m0 1.5c-1.355 0-2.25-.895-2.25-2.25s.895-2.25 2.25-2.25 2.25.895 2.25 2.25-.895 2.25-2.25 2.25m0 6.75c-1.355 0-2.25-.895-2.25-2.25s.895-2.25 2.25-2.25 2.25.895 2.25 2.25-.895 2.25-2.25 2.2m-6.75 0c-1.355 0-2.25-.895-2.25-2.25s.895-2.25 2.25-2.25 2.25.895 2.25 2.25-.895 2.25-2.25 2.25M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    </div>
