...
<x-app-layout title="Jadwal Maintenance">
    <div x-data="{
        ...
        @php
            $totalPc = $pcs->count();
            $countAntrean = \App\Models\ItMaintenanceSchedule::where('status', 'antrean')->count();
            $countDiproses = \App\Models\ItMaintenanceSchedule::where('status', 'diproses')->count();
            $countSelesai = \App\Models\ItMaintenanceSchedule::where('status', 'selesai')->count();
        @endphp
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition">
                                ...
                                @if($latest)
                                    <td class="px-5 py-3.5">
                                        <span class="inline-flex items-center justify-center rounded-lg bg-blue-50 px-2.5 py-1 text-sm font-bold text-blue-600 dark:bg-blue-900/20 dark:text-blue-400">#{{ $latest->urutan }}</span>
                                    </td>
                                    <td class="px-5 py-3.5 text-gray-700 dark:text-gray-300">{{ $latest->tanggal?->format('d M Y') ?? '-' }}</td>
                                    <td class="px-5 py-3.5">
            ...
            <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-cloak>
                ...
                <div class="relative w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900" @click.stop>
                    ...
                    <form action="{{ route('it.maintenance.store') }}" method="POST" class="mt-5 space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Urutan Maintenance</label>
                            <input type="number" name="urutan" x-model="formUrutan" min="1" placeholder="Contoh: 1, 2, 3..." required class="mt-1 block w-full rounded-xl border-gray-200 bg-gray-50 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
                        </div>
                        <div>
                ...
                <div class="relative w-full max-w-lg rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900" @click.stop>
                    ...
                    <form :action="'/it/maintenance/' + editTarget.id" method="POST" enctype="multipart/form-data" class="mt-5 space-y-4">
                        ...
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Urutan Maintenance</label>
                                <input type="number" name="urutan" x-model="editTarget.urutan" min="1" placeholder="Contoh: 1, 2, 3..." required class="mt-1 block w-full rounded-xl border-gray-200 bg-gray-50 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
                            </div>
                            <div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal Maintenance <span class="text-gray-400">(opsional)</span></label>
                            <input type="date" name="tanggal" x-model="editTarget.tanggal" class="mt-1 block w-full rounded-xl border-gray-200 bg-gray-50 text-sm focus:border-blue-500 focus:ring-blue-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
                        </div>
                        <div>
